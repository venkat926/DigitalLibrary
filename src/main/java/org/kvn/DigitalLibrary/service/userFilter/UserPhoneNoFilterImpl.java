...
@Component
public class UserPhoneNoFilterImpl implements UserFilterStrategy{
    ...
    public List<User> getFilteredUsers(Operator operator, String value) {
        if (operator.equals(Operator.EQUALS)) {
            User user = userRepository.findByPhoneNo(value);
            return Collections.singletonList(user);
        } else {
            throw new IllegalArgumentException("Only EQUALS operation is allowed for PhoneNo");
        }
    }
}
